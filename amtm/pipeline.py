...
if TYPE_CHECKING:
    ...
    from .models import RelationClassificationTransformerModule, SequenceTaggingTransformerModule
...
def relation_classification(
    ...
    if tokenizer is None:
        ...
        logger.warning(
            "There was no tokenizer information given, defaulting "
    ...
    tokenized_components = tokenizer(
        text=text,
        text_pair=text_pair,
        padding=True,
    ...
    if return_confidence:
        return [
            {"label": id2label[prediction], "confidence": confidence}
...
def _sentence_annotation(
    ...
    for i, (token, prediction, wid) in enumerate(zip(tokens, predictions, word_ids)):
        ...
        if not current_annotation:
            ...
            # possibilities
            if (
                label.removeprefix("B-").removeprefix("I-") == current_annotation["label"]
                or label == "O"
            ):
...
def sequence_tagging(
    ...
    id2label = defaultdict(
        lambda: "O", {idx: lbl for idx, lbl in id2label.items() if lbl[0] in "BIO"}
    )
